	...
	useEffect(() => {
		...
		Axios.get(urlp)
			.then(res => {
				...
				if (res.status !== 200) alert("Wrong id of patient");
			})
			.catch(err => {
				alert(err);
			});
		...
		Axios.get(urld)
			.then(res => {
				...
				if (res.status !== 200) alert("Wrong id of doctor");
			})
			.catch(err => {
				alert(err);
			});
		...
		Axios.post(
			...
			},
			{
				headers: {
					Authorization:
						"Bearer " +
						"YzZjZGM3NzEtMDdjZi00NzA1LWFjYjUtYzVhYTY1NjE5ZWFlOWI4MjMxMjQtNjVh_P0A1_caaa8419-6ffd-4ddb-8fb5-d89f059408c4",
				},
			}
		)
			.then(res => {
			...
			>
				...
				>
					<FormControl component="fieldset">
						...
						<RadioGroup aria-label="slots" name="slots1" value={slotRadio} onChange={handleRadioChange}>
							<FormControlLabel
								value="09:00:0010:00:00Slot 1"
								disabled
								control={<Radio />}
								label="Slot 1 (9am - 10 am)"
							/>
							<FormControlLabel
								value="10:00:0011:00:00Slot 2"
								control={<Radio />}
								label="Slot 2 (10am - 11 am)"
							/>
							<FormControlLabel
								value="11:00:0012:00:00Slot 3"
								control={<Radio />}
								label="Slot 3 (11am - 12 pm)"
							/>
							<FormControlLabel
								value="16:00:0017:00:00Slot 4"
								control={<Radio />}
								label="Slot 4 (16pm - 17 pm)"
							/>
							<FormControlLabel
								value="17:00:0018:00:00Slot 5"
								control={<Radio />}
								label="Slot 5 (17pm - 18 pm)"
							/>
						</RadioGroup>
					</FormControl>
